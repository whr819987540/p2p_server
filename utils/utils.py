...
def load_dataset(dataset: str, data_dir: str, batch_size: int, indices=None):
    if dataset == "mnist":
        if indices is None:
            raise ValueError("indices is None")
        load_mnist(data_dir, batch_size, indices)
...
def load_mnist(data_dir: str, batch_size: int, indices):
    """
        indices is generated by server and then transfered to clients.
    """
    # 定义数据预处理方式
    ...
    train_dataset = LocalDataset(train_dataset, indices)
